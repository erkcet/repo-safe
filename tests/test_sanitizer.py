...
def test_sanitize_rejects_unicode_normalized_path_aliases_atomically(
    ...
    aliases = [source / composed, source / decomposed]
    monkeypatch.setattr("repo_safe.sanitizer.collect_tree", lambda *args, **kwargs: aliases)
    monkeypatch.setattr("repo_safe.sanitizer.is_link_like", lambda path: False)
